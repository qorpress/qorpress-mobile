...
export default {
  ...
  created() {
    axios.get('https://eedama.org/wp-json/wp/v2/categories/'+this.$route.params.id)
    .then((response) => {
            this.cat = response.data;
            // Get Posts By Category
            axios.get('https://eedama.org/wp-json/wp/v2/posts?categories='+this.$route.params.id+'&_embed')
            .then((response) => {
